...
|---|---|
| [handoff-vbp-atlas-integration.md](handoff-vbp-atlas-integration.md) | **Value-based pricing — shipped into HELIX checkout settings, not Atlas.** Read it for the count that redirected the work: **`edge-checkout-v2-1` ships on exactly one website and it is Storagely's own sandbox**, so the Atlas readers the brief was built around live in a component no legacy operator runs. Also the delta baseline (arrived vs cheapest — same rates, opposite offers), the `/mo` on a four-week price that no operator could fix because the prop was declared nowhere, tier-conditioned add-ons including the `!hasVbpTier()` clause, and four bugs a lint or test gate caught that reading did not |
| [prompt-vbp-atlas-integration.md](prompt-vbp-atlas-integration.md) | **Value-based pricing — moving the legacy handlers into Atlas.** A brief, not a handoff: what the three surviving `*VbpHandler` subclasses do (Columbia, Mini-Mall, Safeguard), what the v4 contract + Atlas block already cover, and nine slices in order — starting with reconciling prod against this clone. **Superseded by the handoff above** — the target moved to Helix mid-session, and **three of its premises are wrong** (S1's whole argument, S5's third token, S6's units-lane start). Every slice is annotated in place with what happened |
